...
st.markdown("경희대학교 처무규정(2025.12.06. 시행)을 기반으로 사용자의 제안을 분석하여 **진성 제안 여부**와 **담당 부서**를 자동으로 분류하는 백엔드 엔진입니다.")

# 3. 사용자 입력창 만들기
user_input = st.text_area("제안 내용을 입력하세요:", height=150, placeholder="예: 총학생회 선거를 전자투표로 바꿀 수 있게 인포21에 기능을 넣어주세요.")

# 4. 분석 버튼 클릭 시 실행될 로직
if st.button("AI 분석 실행"):
    if user_input:
        with st.spinner("AI가 경희대 처무규정을 바탕으로 꼼꼼하게 분석 중입니다..."):
            try:
                # 제미나이 모델 설정 (Gemini 2.5 Flash 적용 및 JSON 출력 강제)
                model = genai.GenerativeModel(
                    ...
                    아래 제시된 [경희대학교 부서별 실제 업무분장(처무규정)]을 완벽하게 숙지하고, 이를 엄격하게 적용하여 사용자의 제안을 가장 적합한 부서로 분류해라.
                    ...
                    [분류 및 라우팅 엄격한 규칙]
                    - 입력된 텍스트가 위 규정과 연관된 '제도적, 인프라적, 행정적 개선안'인지 먼저 판단한다. 단순 시설물 파손이나 즉각 조치가 필요한 건(예: '강의실 에어컨 고장', '변기 막힘')은 총무관리처로 분류하되, 진성 제안이 아니므로 'is_valid_proposal'을 false로 반환한다.
                    - 정보시스템과 관련된 제안일 경우: '전산 시스템의 단순 에러나 인프라 문제'는 정보처를 주관 부서로 매핑한다. 단, '인포21에 새로운 기능/버튼 추가, 기존 행정 규칙(장학, 성적 등) 전산 반영' 등 현업 부서의 정책과 전산 개발이 동시에 필요한 경우, 반드시 정책 주관 부서(예: 교무처, 미래혁신원 등)를 'department'에 넣고, 실제 개발을 수행할 '정보처'를 'collaborative_departments'에 필수로 포함시킨다.
                    - 복합 사안인 경우(예: '외국인 유학생 전용 장학금 전산 시스템 신규 구축' ➔ 주관: 국제처, 협조: 미래혁신원, 정보처), 가장 주도적인 책임 부서를 'department'에 명시하고, 협조 부서를 'collaborative_departments'에 배열 형태로 넣는다.

                    [출력 JSON 스키마 (반드시 이 형태만 유지)]
                    {
                        "is_valid_proposal": boolean,
                        "category": "string (분류 항목)",
                        "department": "string (가장 적합한 주관 부서명)",
                        "collaborative_departments": ["string", "string"],
                        "confidence_score": integer (0~100 사이의 신뢰도 점수),
                        "reasoning": "string (선택한 주관/협조 부서들의 실제 규정에 근거한 상세 매핑 사유)",
