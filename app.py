        ...
        with st.spinner("AI가 처무규정과 V.O.I.C.E 기준을 바탕으로 분석 중입니다..."):
            try:
                ...
                model = genai.GenerativeModel(
                    ...
                    [분류 및 라우팅 엄격한 규칙]
                    - 입력된 텍스트가 위 규정과 연관된 '제도적, 인프라적, 행정적 개선안'인지 먼저 판단한다. 즉각 조치가 필요한 단순 불만(예: '강의실 에어컨 고장')은 총무관리처로 분류하되, 진성 제안이 아니므로 'is_valid_proposal'을 false로 반환한다.
                    - 정보시스템과 관련된 제안일 경우: '전산 시스템의 단순 에러나 인프라 문제'는 **정보처**를 주관 부서로 매핑한다. 단, '인포21에 새로운 기능/버튼 추가, 기존 행정 규칙(장학, 성적 등) 전산 반영' 등 현업 부서의 정책과 전산 개발이 동시에 필요한 경우, 반드시 정책 주관 부서(예: 교무처, 미래혁신원 등)를 'department'에 넣고, 실제 개발을 수행할 '정보처'를 'collaborative_departments'에 필수로 포함시킨다.
